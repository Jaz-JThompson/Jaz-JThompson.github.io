...
Contamination results in the curves no longer coinciding when the voltage sweeps from positive to negative. Low-density plasmas result in an very high SNR as not as many electrons are collected. This results in messy data and no intermediate region where the logarithm can be fit and no inflection point so the $V_p$ cannot be as accurately found. 
